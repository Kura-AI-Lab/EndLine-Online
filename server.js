...
// ★ 重複禁止：ai-core は読み込まず、ai-core-node に統一
...
function initState(mode) { // '2v2_8x8' or '1v1_6x6'
  ...
  if (playerCount >= 2) {
    placeLine(board, size, 0, 0, true, piecesPerSide);
    placeLine(board, size, 1, size - 1, true, piecesPerSide);
  }
...
io.on('connection', (socket) => {
  ...
  socket.on('game:move', ({ roomId, move }) => {
    ...
    if (seatOwner !== socket.id) return;  // 自分の席でない
